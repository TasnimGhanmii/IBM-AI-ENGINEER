...
"""
     This code is essentially linear algebra that simulates the operations of a neural network, specifically an attention mechanism.

It demonstrates the flow of information: input → query → attention → weighted output → decode.

It’s educational: you can see exactly what happens at each step.

But no training occurs, so it doesn’t learn or generalize beyond the dictionary.   
...
def plot_embdings(my_embdings, name, vocab):
    fig = plt.figure()
    # creates one 3D subplot.
    # 111 means “1 row, 1 column, first plot”.
    # projection='3d' tells Matplotlib we want a 3D scatter plot.
    ax = fig.add_subplot(111, projection='3d')

    # Plot the data points
    """my_embdings is assumed to be a 2D NumPy array of shape (num_words, 3).
       Each row = embedding of one word.
       Each column = X, Y, Z coordinate in 3D.

       [:,0] → first column (x-axis),
       [:,1] → second column (y-axis),
       [:,2] → third column (z-axis).
       This draws a dot for each word in 3D.
       the word is gonna be of 3 dim with each column coresponding to 1 axe"""
...
###enhancement in case the key is not in the dictionnary###
...
###Transitioning from basic translation to neural networks###

##define vocabulary##
# Create and sort the input vocabulary from the dictionary's keys
# Wrap the keys in set() → removes any duplicates.
# Even though keys in a dictionary are already unique by definition, this ensures no duplicates if you built it from something messy.
# then Converts that set back into a Python list
# "vocabulary_in is a sorted list of all unique keys in the dictionary."
...
class Head(nn.Module):
    """ Self attention head. This class implements a self-attention mechanism
        ...
        there is no real trainable neural network here.
